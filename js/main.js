 ...
 *
 * 対象ブラウザは現行のモダンブラウザ。想定外の環境で例外が出た場合は、
 * 演出を諦めて全要素を表示する（本文が隠れたままにならないことを最優先）。
 */
(function () {
  'use strict';
  ...
  try {
    ...
    /**
     * @param {boolean} open
     * @param {boolean} [returnFocus=true] 閉じる際にトグルボタンへフォーカスを戻すか（リンク遷移時は戻さない）
     */
    ...
    if (toggle && nav) {
      ...
      // ヘッダー内のリンク（ナビ項目・CTA・ロゴ）を押したらドロワーを閉じる。フォーカスは遷移先に委ねる
      (headerInner || nav).addEventListener('click', function (e) {
